...
let temperatureC = 20;
let temperatureF = 68;
let temperatureFtoC = ((temperatureF - 32) * 5) / 9;
console.log(temperatureFtoC);
let temperatureCtoF = ((temperatureC * 9) / 5) + 32;
...
function calculateFtoC(temperatureF) {
    temperatureFtoC = ((temperatureF - 32) * 5) / 9;
    results.innerHTML = (temperatureF + "F " + "je " + temperatureFtoC.toFixed(2) + " C");
    return temperatureFtoC;
}
...
function calculateCtoF(temperatureC) {
    let temperatureCtoF = ((temperatureC * 9) / 5) + 32;
    results.innerHTML = (temperatureC + "C " + "je " + temperatureCtoF.toFixed(2) + " F");
...
function numberProportion(firstArg, secondArg) {
    let sentence = ``;
    if (secondArg == 0)
        sentence = "Go away hacker u can't divide by 0"

    else {
        res = firstArg / secondArg * 100;
        sentence = `${firstArg} je ${res.toFixed(2)}% z ${secondArg}`;
    }
...
const buttonNumberProportion = document.createElement('button');
buttonNumberProportion.innerText = "PodilCisel (first 2 args)";
buttonNumberProportion.setAttribute('id', 'task-4');
buttonNumberProportion.addEventListener('click', () => { numberProportion(arg1, arg2) });
const tasks4 = document.querySelector('#tasks');
tasks.appendChild(buttonNumberProportion);
 ...
 */
// Solution here
function compare(firstArg, secondArg) {

    if (firstArg > secondArg) {
        results.innerHTML = firstArg;
        return firstArg;
    }
    if (firstArg < secondArg) {
        results.innerHTML = secondArg;
        return secondArg;
    }
    else {
...
buttonCompare.innerText = "Kdo koho? (only for the first two args)";
buttonCompare.setAttribute('id', 'task-5');
buttonCompare.addEventListener('click', () => { compare(arg1, arg2) });
...
buttonCircleArea.innerText = "Around and about: obsah kruznice (first arg is polomer)";
buttonCircleArea.setAttribute('id', 'task-7');
buttonCircleArea.addEventListener('click', () => { circleArea(arg1) });
...
function calculateConeVolume(radius, height) {
    ...
    var volume =1/3 * circleArea(radius) * height;
    results.innerHTML = volume;
    return volume;
}
//console.log(objemKuzelu(1, 2));

const buttonCalculateConeVolume = document.createElement('button');
buttonCalculateConeVolume.innerText = "Objem kuzelu (first arg is polomer, druhy je vyska)";
buttonCalculateConeVolume.setAttribute('id', 'task-8');
buttonCalculateConeVolume.addEventListener('click', () => { calculateConeVolume(arg1, arg2) });
