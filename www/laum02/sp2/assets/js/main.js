(() => {
    ...
    $(".input").submit((e) => {
        ...
        let end = false;
        // Try to fill user
        request(create_user_url(user), fill_user).catch((r) => {
            end = true;
            if (r.status == 429) {
                show_error(missingUserElement, "Poslali jste požadavků. Zkuste to prosím znovu za hodinu.")
            } else if (r.status == 403) {
                show_error(missingUserElement, "Je nutné aktivovat cors-anywhere. Klikněte prosím na odkaz v zápatí stránky.")
            } else if (r.status == 400) {
                show_error(missingUserElement, "Jméno nesplňuje požadavky twitteru.")
            } else {
            ...
            // Get tweet
            request(create_latest_tweet_url(userId), fill_latest_tweet);
        });

    });

    // Create URL for user request
    function create_user_url(user) {
    ...
    // Fill tweet
    function fill_latest_tweet(tweet) {
        if (tweet == "") {
            show_error(missingTweetElement, "Uživatel ještě nikdy netweetnul:(.")
        } else {
